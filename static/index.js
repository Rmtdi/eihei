...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;迪老爷把你们踢出酒馆！';
        gameOver();
...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '最多次数&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    if (score == 99) return '常常酒酒～';
    if (score <= 9) return '就这？你该不会是没有买酒的摩拉了吧';
    if (score <= 49) return '他还没醉！';
    if (score <= 100) return '琴已经拿不动了';
    if (score <= 149) return '扑腾，昏睡倒地';
    return '快叫芭芭拉来！';
}
